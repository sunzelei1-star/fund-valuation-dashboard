...
- 额外会输出 `est_nav_fallback_rows=...`，用于确认 `est_nav` 缺失回退逻辑可用；
...
- 历史趋势：`akshare_live` 下趋势图优先使用 `ak.fund_open_fund_info_em(..., indicator="单位净值走势")` 的真实净值历史；若估值历史接口不可用，则自动降级为 `Est NAV = NAV` 并在页面提示。
- 估值语义：统一区分 `official_estimate / approx_from_change / nav_snapshot`，并在快照表、KPI、持仓分析、图表 hover 中保持一致。
- 持仓输入模式：支持两种方式：
  - 专业模式：`份额 + 成本单价`
  - 简化模式：`持有金额 + 持有收益`（系统自动推导份额、成本、市值、收益率）
- 多账户：支持新增/删除账户、切换单账户视角与全部账户汇总视角，并展示账户级 KPI。

### 使用多账户与双模式

1. 在左侧“持仓与账户管理”新增账户（如主账户、支付宝、家庭账户）。
2. 选择账户后可新增基金，录入模式可选 `simple` 或 `pro`。
3. 切换到“全部账户汇总”可查看汇总 KPI 与分析图表。
